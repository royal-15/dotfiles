...
def _first_drawing_coordinate(hpgl):
    """Return the first coordinate at which the pen actually starts drawing.

    The first PU coordinate is only a positioning move and is not necessarily
    the start of the first cut. The return-to-start position must therefore be
    taken from the first PD coordinate.
    """
    command_re = re.compile(r"PD([^;]*);", re.IGNORECASE)
...
class SkycutC24HpglOutput(inkex.OutputExtension):
    ...
    def save(self, stream):
        ...
        # Capture the first actual PEN-DOWN coordinate before transformation.
        # This is the exact point where the first cut begins, and therefore
        # the point to which the carriage must return after cutting.
        start_point = _first_drawing_coordinate(hpgl)
        ...
        # IMPORTANT: return to the exact transformed FIRST-DRAWING coordinate.
        # The initial PU is deliberately ignored because it only positions the
        # carriage. Do not use PU0,0: the carriage must finish at the first
        # pen-down point.
